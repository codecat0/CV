...
class DecodeBox(object):
    ...
    def decode_box(self, inputs):
        """
        :param inputs: 输入的input一共有三个，他们的shape分别为
                        batch_size, 3 * (25 = 5 + num_classes), 20, 20
                        batch_size, 3 * 25, 40, 40
                        batch_size, 3 * 25, 80, 80
        """
        outputs = []
        ...
        for i, input in enumerate(inputs):
            ...
            ).view(x.shape).to(device)
            grid_y = torch.linspace(0, input_height-1, input_height).repeat(input_width, 1).t().repeat(
                batch_size * len(self.anchors_mask[i]), 1, 1
            ).view(y.shape).to(device)
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_threshold=0.5, nms_threshold=0.4):
        ...
        for i, image_pred in enumerate(prediction):
            # 对种类预测部分取max，class_conf: 所属种类概率 (num_anchors, 1), 类别：(num_anchors, 1)
